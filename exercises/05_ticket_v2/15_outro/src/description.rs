...
impl TryFrom<String> for TicketDescription {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        if value.is_empty() {
            return Err(Error::new(std::io::ErrorKind::InvalidInput, "The description cannot be empty"));
        }
        if value.len() > 500 {
            return Err(Error::new(std::io::ErrorKind::InvalidInput, "The description cannot be longer than 500 characters"));
        }
